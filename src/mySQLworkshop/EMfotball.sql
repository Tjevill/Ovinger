...
SELECT mdate, et1.teamname, et2.teamname
FROM (game INNER JOIN eteam et1 ON (team1=et1.id))INNER JOIN eteam et2 on (team2=et2.id)
ORDER BY game.mdate;

-- Finn teamname for begge lagene og totalt antall mål i hver kamp ,
-- inkl. de kampene som endte 0-0 (hvor antall mål er 0), sortert stigende etter når kampen ble spillt.
SELECT mdate,et1.teamname, et2.teamname,count(gtime)AS antall_maal
FROM (game  INNER JOIN eteam et1 ON (team1=et1.id))INNER JOIN eteam et2 ON (team2=et2.id)
